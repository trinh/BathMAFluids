...
},
{
  ...
  "body": " Problems set 7 to hand in   Please hand in the problems in this Section for Set 7 (due in the week commencing 1 Dec).    Analysis of flow in a pipe    It might help to try before attempting this problem.   A viscous incompressible fluid flows steadily along an infinitely long pipe of radius under a constant pressure gradient . Work in cylindrical polar coordinates with corresponding velocity components with the axis of the pipe aligned with .     Write down the boundary conditions to be applied.       The velocity is steady and therefore independent of . Explain why it is also reasonable to assume that the velocity profile is independent of and , and also that the -component of the velocity is zero.       Use the continuity equation to show that the -component of the velocity is also zero ( ).  In cylindrical polar coordinates, the continuity equation for an incompressible fluid is        Hence the only non-zero component of the velocity is the -component, , and this only depends on , . Check that the profile satisfies the boundary conditions.  Note that we also need to check that the velocity is smooth at . This is easier to see if we rewrite the velocity profile in Cartesian coordinates. Converting to Cartesian coordinates, the flow is with which is smooth at . Note, however, that for example the profile would not be smooth at .       The velocity profile increases as increases and decreases as increases. Explain why this is to be expected. Thus, although you cannot yet prove this is the correct velocity profile (you will soon be able to do so), this profile does look reasonable from what you already know.       Calculate the three components of the stress vector, the force per unit area that the fluid exerts, on the inside of the pipe. You may assume that the pressure equals at . You may also assume that the rate-of-strain tensor in cylindrical polar coordinates is given by , and that, being a Newtonian fluid, the stress tensor is given by .       You should have found that the normal stress is radially outward, while the shear stress points along the pipe in the direction of increasing . Explain why you would expect these directions.       Rotating flow    Based on Exercise 6.6 in .   Consider an incompressible Newtonian fluid flowing with velocity , where is a constant vector. Since is constant, for simplicity you may assume that .     Calculate the velocity components in Cartesian coordinates in the case .       Show that this flow is incompressible.       Find the components of the stress tensor.       Without doing a calculation, explain why you would have expected this answer.       Flow outside a rotating cylinder    Based on problem 2.8 in .   An incompressible Newonian fluid fills . An infinite solid cylinder of radius is placed in the fluid with its axis on the line in cylindrical polar coordinates.     The cylinder rotates at constant angular velocity about its axis. In the resulting flow, explain why it is reasonable to assume that and .       Hence perform a calculation to show that .  You may use the continuity equation in cylindrical polar coordinates:        The Navier Stokes equations in cylindrical polar coordinates are where the Laplacian is defined by its action on a function : and represents the body forces per unit mass acting on the fluid, which you may neglect in this question.  Show that the flow satisfies the Navier Stokes equations and boundary conditions.       If the fluid and cylinder are initially at rest, and the cylinder suddenly starts rotating with angular velocity at time , explain qualitatively how you would expect the flow to evolve with time.      "
},
{
...
},
{
  ...
  "number": "7.10.2",
  "title": "Rotating flow.",
  "body": " Rotating flow    Based on Exercise 6.6 in .   Consider an incompressible Newtonian fluid flowing with velocity , where is a constant vector. Since is constant, for simplicity you may assume that .     Calculate the velocity components in Cartesian coordinates in the case .       Show that this flow is incompressible.       Find the components of the stress tensor.       Without doing a calculation, explain why you would have expected this answer.     "
},
{
