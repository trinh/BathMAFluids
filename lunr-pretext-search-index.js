...
},
{
  ...
  "number": "3.1.7",
  "title": "",
  "body": " Reynolds' Transport Theorem is particularly useful when the integral quantity is a conserved quantity . A conserved quantity is something for which we can apply a principle of conservation . In practice for this course, this is usually mass or linear momentum:   Mass: and the left-hand side of is zero because mass is conserved.    Linear momentum: is now a vector quantity and the left-hand side of equals the sum of the all the forces acting on the fluid in the control volume , which are:   Surface forces: Forces acting on the surfaces of the fluid, which could be reaction forces from the walls of a container, or forces due to pressure or stress from surrounding fluid. (Note that we will cover stress in .)    Body forces: Forces acting over the interior of the fluid, and in this course you will usually only need to consider the force of gravity, although in principle other body forces could act such as an electromagnetic force. In addition, if working in a noninertial frame of reference, other apparent forces need to be added (forces due to linear acceleration, angular acceleration and the Coriolis and centrifugal forces).         "
},
{
  "id": "subsec-RTT-6-1",
  "level": "2",
  ...
  "number": "3.1.8",
  "title": "",
  "body": "  Non-examinable: A more general statement of the Reynolds' Transport Theorem is as follows: We define a control volume  . This is a region of the fluid that we specify; it could be anything and in particular it doesn't need to follow the fluid particles (unlike ). Then the rate of change of in the system of particles equals the integral of the rate of change of over the control volume plus the net flux of the through the surface of the control volume: Here, is the amount of in the system of particles, that is the Lagrangian derivative of , is the amount of in the control volume and is the flux of across the surface of the control volume. Note that, if is conserved then we can write the left-hand side in terms of other things (zero if is mass and the sum of the forces if is linear momentum).  "
},
{
