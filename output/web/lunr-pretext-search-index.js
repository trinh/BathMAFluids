...
},
{
  ...
  "number": "6.2.13",
  "title": "Vortex pair.",
  "body": " Vortex pair  We assume two-dimensional flow, and consider the evolution of two line vortices of equal and opposite strengths initially a distance apart at locations with the vortex at having strength and that at having strength .  The vortex at will move under the influence of that at with speed in the -direction. Similarly, the vortex will also move with speed in the -direction. Since their separation remains constant, the two vortices will continue to move at a constant speed in the -direction.  In the frame of reference moving with the vortices, there is a uniform flow in the -direction and the flow due to each of the vortices. The complex potential is the sum of the complex potentials due to these three elements: The streamfunction is the imaginary part of this:   To help with plotting the streamfunction, we note that:   The derivative of the complex potential is which equals zero at the points , so these are stagnation points of the flow.    We have on the line , so this is a streamline.    Far from the vortices (in any direction), the streamfunction is approximately , so the streamlines are approximately vertical lines.    As we approach the vortex at , tends to and as we approach the vortex at , tends to .     These points imply that there is a curve on which in the right-half plane. This curve joins the two stagnation points and goes around the vortex. Similarly, there is a curve on which in the left-half plane, and these two curves are reflections of one another.  If we dye the fluid within these two curves, we would create a blob of fluid surrounding the vortices. An observer would see this blob of fluid travelling at the speed in the -direction without changing shape, see Figure b.   Flow due to a vortex pair relative to (a) a fixed frame, and (b) a frame moving with the vortices. Note that the shaded region in (b) represents a fluid region that, if it were dyed, the dyed fluid would remain in the same region throughout the evolution of the flow. From .   Vortex pair    "
},
{
...
{
  "id": "sec-axisymmetric-flow-2-4",
  "level": "2",
  "url": "sec-axisymmetric-flow.html#sec-axisymmetric-flow-2-4",
  "type": "Remark",
  "number": "6.3.2",
  "title": "",
...
},
{
  ...
  "number": "6.5",
  "title": "Exercises",
  "body": "   **Edits w\/c 1 Dec**: we have included some more commentary in the solutions, and are more precise at the start of questions which questions are examinable and which ones are more for deepening your knowledge of the material. Some additional notes have been added into the solutions based on the submitted problem sets of students.  We would suggest the following priorities:   Q1, 2, 7, 8: these were hinted at in lectures.    Q6: good to do as this is connected to previous material.    Q3--Q5: not examinable        Kelvin’s Circulation Theorem   The circulation around a closed curve is defined by By transforming to Lagrangian variables show that is independent of . Deduce that a flow that is initially irrotational (i.e. at ) remains irrotational for all time.  Note that this is a key result: if a flow is initially irrotational, it remains so indefinitely and we can introduce a velocity potential such that .     Method one: Consider the following diagram.   Curve at times and . The inset shows the relationship between the two curves.    By definition, We therefore need to write and at time on the curve to values at time on the curve :    Method two: We can parametrise via for and rewrite the integral as an integral over the parameter : Then start from and simplify.    We present two methods.   Method one:   Assuming a conservative body force per unit volume, the Euler equation reads We have We calculate Hence The final equality above follows because the line integral of a gradient around a closed curve is zero. Thus is independent of . If the flow is initially irrotational, then at for any closed curve , and hence for all time. Since this holds for any closed curve, it follows that for all time.   Method two: This can also be proven by parametrising via for and rewriting the integral as an integral over the parameter : Then      The circulation around a vortex tube   (Based on Exercise 5.5 in ). Define a vortex tube. Prove that the quantity where is any surface spanning the tube, is the same for all such surfaces. Relate this quantity to a circulation of the flow.    Given two different such surfaces and , define a `cylindrical' volume , whose ends are the the two surfaces and and whose sides are the sides of the vortex tube, and let , for .    Given two different such surfaces and , define a `cylindrical' volume , whose ends are the the two surfaces and and whose sides are the sides of the vortex tube, and let , for .  Consider . This is equal to minus plus the integral of over the sides of the cylinder. On the sides of the cylinder, we have . Hence, for any two such surfaces.  Stokes theorem implies that where is the curve around the edge of , which is the circulation.  Since the circulation is constant in time, this implies the flux of vorticity through any vortex tube is uniform and independent of time.     Evolution of a line integral (non-examinable)   (Based on Exercise 5.6 in ). Show that if is any suitably smooth vector field and where is a circuit consisting of the same fluid particles as time proceeds, then     Similar to , there are two methods of solution possible.   First method:    Curve at times and . The inset shows the relationship between the two curves.    We write    Second method: Parametrise as for . Then     We present two methods of solution.   First method:    Curve at times and . The inset shows the relationship between the two curves.    We write and note that Hence Now we use the identity to expand Hence whence the result.   Second method: Parametrise as for . Then Then We can rewrite the expression we are aiming for: We use the identity and   Hence, whence the result.     Axisymmetric flow (non-examinable)    Please feel free to omit this problem as we have not covered the material.   (Based on Exercise 5.7 in ). In this question, please make use of the vector calculus formulae given in cylindrical polar coordinates, available at this website . Consider an axisymmetric flow. Working in cylindrical polar coordinates , show that the vorticity is given by and that the vorticity equation reduces to Thus the vorticity changes in proportion to distance from the axis of symmetry.  Note that in spherical polar coordinates this is equivalent to     Axisymmetry implies that has no -component, that is , and that . Hence, .  Then, using the formula for curl in cylindrical polar coordinates,   We have Show that and use     Axisymmetry implies that has no -component, that is , and that . Hence, .  Then, using the formula for curl in cylindrical polar coordinates, where .  We have We note that and has gradients with respect to and (and not with respect to ), meaning that . Also, since , Hence, the only non-zero component of the vorticity equation is the -component, and this reads:   Now, and and hence   Dividing the vorticity equation by : as required.     Two vortices (non-examinable)   Two vortices, of strengths and , are at the points and , respectively, in the complex plane. Write down the equations of motion for the position vectors and if the vortices are free to move. Assuming that , show that , where is the centroid of the two vortices, and is the distance between them.  Deduce that each vortex moves in a circle centred on , with angular velocity What happens in the exceptional case where ?    The complex potential for the two vortices is Hence, the velocity field is given by The velocity of each vortex is given by evaluating this expression at the position of the other vortex.    The complex potential for the two vortices is Hence, the velocity field is given by The velocity of each vortex is given by evaluating this expression at the position of the other vortex.  Thus, the equations of motion for the vortices are   We calculate Similarly,   Thus, and are constant in time. To find the angular velocity, we set      Flow past a circle with a vortex   Fluid occupies the region outside a circular obstacle of radius . By using the Milne-Thomson Circle Theorem, find the resulting complex potential when a vortex of strength is placed at , where (assuming there to be no circulation about the obstacle).  If the vortex moves in a circle of radius , show that the angular velocity is of magnitude   It is helpful to note that the relationship between instantaneous (linear) velocity, say and angular velocity is for an object rotating around a circle of radius .    The complex potential associated with a vortex in isolation at the point is The Milne-Thomson Circle Theorem states that, for a complex potential , the function has the same singularities as outside the circle and that is a streamline of the flow with complex potential . Removing the line vortex from the associated flow allows its velocity to be found.    The complex potential associated with the vortex in isolation is We construct the complex potential in the presence of the obstacle using the Milne-Thomson Circle Theorem: separating the logarithms in the final step enables the derivative to be taken more easily. This has a single singularity at the vortex and the circle is a streamline, as required.  The velocity field can be found by taking the derivative of : and, removing the vortex (the term proportional to ), the velocity of the vortex at is which is an instantaneous velocity of magnitude upwards.  By symmetry, the vortex moves in a circle of radius about the origin with speed equal to this value. We thus divide the above by the radius , giving an angular velocity of This is negative, so the line vortex goes clockwise around the origin with angular velocity as required.   Why is angular velocity equal to linear velocity divided by the radius? Consider a particle given by Then . Hence (or the angular velocity) is equal to the magnitude of the (linear) velocity divided by .      Flow in a quadrant   Fluid occupies the quadrant , bounded by two rigid boundaries along the - and -axes. Find the complex potential for the flow caused by a vortex of strength at a point in the fluid. If the vortex is free to move, show that it follows a path on which      See also .   Suppose the vortex is at the point We use three image vortices at and at , both of strength , and at of strength .  Recall that the complex potential of a line vortex at of strength is and its velocity is given by the derivative of this map: .  The velocity of the vortex is given by the sum of the velocities due to the other three vortices: Hence and therefore which can be solved by separation of variables to give where is constant, and this can be rearranged into the required form.   Alternative method: We could achieve the same solution using a conformal map. The map transforms the quadrant , to the upper half plane . The vortex at transforms to a vortex at with complex potential We need to add an image vortex at of strength . Thus the complex potential in the -plane is This is equivalent to having vortices of strength at , and vortices of strength at , which is the same result as above.   Additional note: We could have tried the transformation to map the quadrant to the whole plane. However, we would need to find the solution for a vortex with a half-line (the positive real axis) removed, which is more complicated than the upper half plane.     Channel flow (Hard)   Fluid occupies the semi-infinite channel . Show that the flow induced by a line vortex of strength at the point has complex potential Show that the velocity components satisfy   Deduce that, if the vortex is free to move, it will instantaneously travel downwards with speed .  You may use the identities and     Consider the map     Consider the transformation The inverse transformation is   With this map, the line transforms to part of the imaginary axis and the constraint corresponds to the line between the points . The lines transform to and the constraint corresponds to the line segments above and below , respectively. Hence the channel is transformed to the right half plane:   Conformal map of channel   Channel map    We have a vortex of strength at the point , which point transforms to . In the -plane, we also require an image vortex of strength at the point . This will ensure that the transformed channel boundary in the -plane (i.e. the imaginary axis) has a no-penetration boundary condition.  Therefore, the complex potential in the -plane is given by Hence in the -plane, the map is   The velocity components are given by the derivative: Using the identity given in the question, we have, as required.  Using the Helmholtz principle, the vortex moves due to the flow of everything but itself. Thus at , excluding the part of the flow due to the vortex, if the vortex flow moves with velocity , we have corresponding to a downward velocity     "
},
{
  "id": "ws-vorticity-1-1",
  "level": "2",
...
},
{
  ...
  "type": "Exercise",
  "number": "6.5.3",
  "title": "Evolution of a line integral (non-examinable).",
  "body": " Evolution of a line integral (non-examinable)   (Based on Exercise 5.6 in ). Show that if is any suitably smooth vector field and where is a circuit consisting of the same fluid particles as time proceeds, then     Similar to , there are two methods of solution possible.   First method:    Curve at times and . The inset shows the relationship between the two curves.    We write    Second method: Parametrise as for . Then     We present two methods of solution.   First method:    Curve at times and . The inset shows the relationship between the two curves.    We write and note that Hence Now we use the identity to expand Hence whence the result.   Second method: Parametrise as for . Then Then We can rewrite the expression we are aiming for: We use the identity and   Hence, whence the result.   "
},
{
  ...
  "type": "Exercise",
  "number": "6.5.4",
  "title": "Axisymmetric flow (non-examinable).",
  "body": " Axisymmetric flow (non-examinable)    Please feel free to omit this problem as we have not covered the material.   (Based on Exercise 5.7 in ). In this question, please make use of the vector calculus formulae given in cylindrical polar coordinates, available at this website . Consider an axisymmetric flow. Working in cylindrical polar coordinates , show that the vorticity is given by and that the vorticity equation reduces to Thus the vorticity changes in proportion to distance from the axis of symmetry.  Note that in spherical polar coordinates this is equivalent to     Axisymmetry implies that has no -component, that is , and that . Hence, .  Then, using the formula for curl in cylindrical polar coordinates,   We have Show that and use     Axisymmetry implies that has no -component, that is , and that . Hence, .  Then, using the formula for curl in cylindrical polar coordinates, where .  We have We note that and has gradients with respect to and (and not with respect to ), meaning that . Also, since , Hence, the only non-zero component of the vorticity equation is the -component, and this reads:   Now, and and hence   Dividing the vorticity equation by : as required.   "
},
{
  "id": "ex-two-vortices",
  "level": "2",
  ...
  "type": "Exercise",
  "number": "6.5.5",
  "title": "Two vortices (non-examinable).",
  "body": " Two vortices (non-examinable)   Two vortices, of strengths and , are at the points and , respectively, in the complex plane. Write down the equations of motion for the position vectors and if the vortices are free to move. Assuming that , show that , where is the centroid of the two vortices, and is the distance between them.  Deduce that each vortex moves in a circle centred on , with angular velocity What happens in the exceptional case where ?    The complex potential for the two vortices is Hence, the velocity field is given by The velocity of each vortex is given by evaluating this expression at the position of the other vortex.    The complex potential for the two vortices is Hence, the velocity field is given by The velocity of each vortex is given by evaluating this expression at the position of the other vortex.  Thus, the equations of motion for the vortices are   We calculate Similarly,   Thus, and are constant in time. To find the angular velocity, we set    "
},
{
  "id": "ex-obstacle",
  "level": "2",
  ...
  "type": "Exercise",
  "number": "6.5.6",
  "title": "Flow past a circle with a vortex.",
  "body": " Flow past a circle with a vortex   Fluid occupies the region outside a circular obstacle of radius . By using the Milne-Thomson Circle Theorem, find the resulting complex potential when a vortex of strength is placed at , where (assuming there to be no circulation about the obstacle).  If the vortex moves in a circle of radius , show that the angular velocity is of magnitude   It is helpful to note that the relationship between instantaneous (linear) velocity, say and angular velocity is for an object rotating around a circle of radius .    The complex potential associated with a vortex in isolation at the point is The Milne-Thomson Circle Theorem states that, for a complex potential , the function has the same singularities as outside the circle and that is a streamline of the flow with complex potential . Removing the line vortex from the associated flow allows its velocity to be found.    The complex potential associated with the vortex in isolation is We construct the complex potential in the presence of the obstacle using the Milne-Thomson Circle Theorem: separating the logarithms in the final step enables the derivative to be taken more easily. This has a single singularity at the vortex and the circle is a streamline, as required.  The velocity field can be found by taking the derivative of : and, removing the vortex (the term proportional to ), the velocity of the vortex at is which is an instantaneous velocity of magnitude upwards.  By symmetry, the vortex moves in a circle of radius about the origin with speed equal to this value. We thus divide the above by the radius , giving an angular velocity of This is negative, so the line vortex goes clockwise around the origin with angular velocity as required.   Why is angular velocity equal to linear velocity divided by the radius? Consider a particle given by Then . Hence (or the angular velocity) is equal to the magnitude of the (linear) velocity divided by .    "
},
{
  "id": "ex-quadrant",
  "level": "2",
  ...
  "body": " Flow in a quadrant   Fluid occupies the quadrant , bounded by two rigid boundaries along the - and -axes. Find the complex potential for the flow caused by a vortex of strength at a point in the fluid. If the vortex is free to move, show that it follows a path on which      See also .   Suppose the vortex is at the point We use three image vortices at and at , both of strength , and at of strength .  Recall that the complex potential of a line vortex at of strength is and its velocity is given by the derivative of this map: .  The velocity of the vortex is given by the sum of the velocities due to the other three vortices: Hence and therefore which can be solved by separation of variables to give where is constant, and this can be rearranged into the required form.   Alternative method: We could achieve the same solution using a conformal map. The map transforms the quadrant , to the upper half plane . The vortex at transforms to a vortex at with complex potential We need to add an image vortex at of strength . Thus the complex potential in the -plane is This is equivalent to having vortices of strength at , and vortices of strength at , which is the same result as above.   Additional note: We could have tried the transformation to map the quadrant to the whole plane. However, we would need to find the solution for a vortex with a half-line (the positive real axis) removed, which is more complicated than the upper half plane.   "
},
{
  "id": "ex-channel",
  "level": "2",
  ...
  "type": "Exercise",
  "number": "6.5.8",
  "title": "Channel flow (Hard).",
  "body": " Channel flow (Hard)   Fluid occupies the semi-infinite channel . Show that the flow induced by a line vortex of strength at the point has complex potential Show that the velocity components satisfy   Deduce that, if the vortex is free to move, it will instantaneously travel downwards with speed .  You may use the identities and     Consider the map     Consider the transformation The inverse transformation is   With this map, the line transforms to part of the imaginary axis and the constraint corresponds to the line between the points . The lines transform to and the constraint corresponds to the line segments above and below , respectively. Hence the channel is transformed to the right half plane:   Conformal map of channel   Channel map    We have a vortex of strength at the point , which point transforms to . In the -plane, we also require an image vortex of strength at the point . This will ensure that the transformed channel boundary in the -plane (i.e. the imaginary axis) has a no-penetration boundary condition.  Therefore, the complex potential in the -plane is given by Hence in the -plane, the map is   The velocity components are given by the derivative: Using the identity given in the question, we have, as required.  Using the Helmholtz principle, the vortex moves due to the flow of everything but itself. Thus at , excluding the part of the flow due to the vortex, if the vortex flow moves with velocity , we have corresponding to a downward velocity    "
},
{
  "id": "definitions",
  "level": "1",
