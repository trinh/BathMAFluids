...
},
{
  ...
  "body": " Decomposition of a shear flow   In this problem, you will apply the equation from the previous problem to a shear flow .     Use to decompose into its local (i) translational, (ii) rotational, and (iii) pure straining components. For (iii), find the eigenvalues and eigenvectors, which give the principle directions of strain. Sketch each of the components (ii) and (iii).  Note that this example is a special case in which the rotational and pure straining components are uniform over the fluid domain. More generally, both will vary.    (ii) The vorticity and therefore the rotational component is which is clockwise rotation with angular velocity , see left sketch in .  (iii) We have the rate-of-strain tensor Hence (note that we are interchanging between and ). The eigenvalues of satisfy which has solutions . The associated eigenvectors are This corresponds to expansion along the line and contraction along , see middle sketch in .  The solution is illustrated in .   Sketches showing the decomposition of the flow into rotation (left) and pure straining flow (middle), with the total flow shown on the right.      "
},
{
